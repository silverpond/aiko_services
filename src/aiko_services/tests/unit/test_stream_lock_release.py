...
def test_create_frames_generator_lock_not_released_on_exception():
    ...
    def run_pipeline():
        with tempfile.NamedTemporaryFile(delete=True, mode="w") as file:
            ...
            pipeline = aiko.PipelineImpl.create_pipeline(
                ...
                grace_time=None, queue_response=response_queue)
    ...
    while True:
        stream_event, _frame_result = response_queue.get(timeout=1.0)
        if stream_event["state"] == StreamEvent.ERROR:
            break
    test_result["pipeline"].stop()
